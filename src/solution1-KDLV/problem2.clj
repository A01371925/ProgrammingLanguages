...
(defn find-unique-evenness-num
  ...
  the other elements."
  [lst]
  (if( = 1 (count (remove even? lst)) )
    [(last (remove even? lst)) (.indexOf lst (last (remove even? lst))) ]
    (if( = 1 (count (remove odd? lst)) )
      [(last (remove odd? lst)) (.indexOf lst (last (remove odd? lst)))]
      )
    )
  )
